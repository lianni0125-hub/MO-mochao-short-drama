...
app.put("/api/settings/embedding",(req,res)=>{
  ...
  const embeddingKeyNames={openai:"OPENAI_EMBEDDING_API_KEY",google:"GOOGLE_EMBEDDING_API_KEY",minimax:"MINIMAX_EMBEDDING_API_KEY",zhipu:"ZHIPU_EMBEDDING_API_KEY",qwen:"QWEN_EMBEDDING_API_KEY",baidu:"BAIDU_QIANFAN_EMBEDDING_API_KEY",custom:"CUSTOM_EMBEDDING_API_KEY"},keyName=embeddingKeyNames[provider];values.set("EMBEDDING_PROVIDER",provider);values.set("EMBEDDING_BASE_URL",baseUrl);values.set("EMBEDDING_MODEL",model);if(keyName&&key)values.set(keyName,key);for(const [savedProvider,envName] of Object.entries(embeddingKeyNames)){const savedKey=savedProvider===provider?key:config.embeddingProviderKeys?.[savedProvider];if(savedKey)values.set(envName,savedKey);}if(provider==="minimax")values.set("MINIMAX_EMBEDDING_GROUP_ID",groupId);
  fs.writeFileSync(envPath,[...values].map(([k,v])=>`${k}=${v}`).join("\n")+"\n",{encoding:"utf8",mode:0o600});
...
app.post("/api/projects/:id/jobs/first-appearances",requireProject,(req,res)=>{const count=get("SELECT COUNT(*) count FROM episodes WHERE project_id=@id AND summary<>''",{id:req.project.id})?.count||0;if(!count)return res.status(400).json({error:"请先生成分集梗概"});res.status(202).json(enqueueJob(req.project.id,"outline_first_appearances","all"));});
...
function reviewedTemplateAnalysis(input,base){
  ...
  return {...base,
    ...
    inferredScriptFormat:{
      episodeHeading:"",sceneHeading:cleanText(raw.sceneHeading,defaults.sceneHeading,160),dialogue:cleanText(raw.dialogue,defaults.dialogue,160),voiceOver:cleanText(raw.voiceOver,defaults.voiceOver,240),specialSpeaker:cleanText(raw.specialSpeaker,defaults.specialSpeaker,160),action:cleanText(raw.action,defaults.action,300),notes:cleanText(raw.notes,defaults.notes,300),
      novelCharacters:numberRange(raw.novelCharacters,{min:1000,ideal:1250,max:1500},{automaticIdeal:true}),targetCharacters:numberRange(raw.targetCharacters,{min:1500,ideal:1750,max:2000},{automaticIdeal:true}),novelAcceptance:numberRange(raw.novelAcceptance||raw.novelCharacters,{min:1000,ideal:1250,max:1500}),scriptAcceptance:numberRange(raw.scriptAcceptance||raw.targetCharacters,{min:1500,ideal:1750,max:2000}),targetScenes:numberRange(raw.targetScenes,{min:1,ideal:2,max:9}),
