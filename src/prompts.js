...
动作只写演员理解并完成这场戏所必需的内容：每场开头用一至两句建立场地规模、关键区域和主要人物的初始位置；人物因剧情进入、离开、接近、阻拦、拖拽或转移到另一位置；关键道具被使用或易手；造成状态变化的身体行动、危险及其可见结果。空间建立不是气氛描写，必须能回答“谁在哪里、正对谁做什么、入口或关键物件在哪里”。能从对白直接听明白的情绪和关系，不再补神态动作；不为每句对白配动作，不写喝水、起身、转头、看向等不改变剧情的过程。对白应占正文信息量的主要部分，有限字数优先给关键交锋，而不是环境和动作。
禁用“指节发白、手指关节发白、青筋暴起、指甲掐进掌心、掌心渗出血丝、手背筋络凸起”等套路化 AI 身体细节。若这类句子不影响事件就直接不写；若其中确有关键动作，只保留会改变事件的朴素动作，不补写另一种神态或身体反应。
...
export function buildStagePrompt(stage, project, constraints, artifacts, evidence = [], range = null) {
  const tasks = {
    ...
    characters: `根据已确认的02故事策划生成核心人物卡。顶层只包含 visual_style 和 characters。若上下文存在 character_brief，它是用户的“人物补充要求”：可以是指定人物、人设灵感、需要AI补全的部分、希望增加的角色或明确的人数要求，必须逐项落实，不得用自己认为更合适的其他角色替换用户指定的性别、定位、身份或关系，也不得借此改写主线。“少了/再加/新增N个”表示在现有人物之外新增N人；“一共/需要N个”才表示最终总数。用户明确点名的人物必须逐一生成；用户明确要求人物数量时必须达到该数量，即使超过8人也不得删减、合并或擅自只选“核心六人”。只有用户既未指定人物也未指定数量时，才根据主线实际需要生成至少8名主要人物；这个默认数量不是上限。首次生成时，先落实用户明确指定的人物和人数，再补齐主线必需的人物。若上下文已有 characters，这是后续重新生成：原有人物是不可删除的既有清单；保留其姓名、核心定位和不冲突设定，只按 character_brief 明确调整或新增人物，不得因为角色定位相同而合并人物。输出前逐句核对 character_brief，确认人数、性别、角色类型、身份和关系均已落实。visual_style 是全剧所有人物共用的视觉基准，必须从02的框架、世界观和故事梗概提取时代地域、核心场域、题材气质、真人短剧质感、统一色彩与光线；写成80–160字，不含任何单个人物特征。characters 数组中每人只包含 role、name、age、personality、biography、image_prompt。personality 简洁具体；biography 写清身份、与主线的关系、全剧作用与结局方向。image_prompt 只写该人物独有且能被画面直接呈现的特征：明确年龄与性别呈现、体型比例、脸型与五官、肤色、发型、符合时代/地域/职业/经济状况的分层服装及材质颜色、鞋履、一个有身份意义的配饰或道具、克制而明确的站姿与表情；不得用“善良、腹黑、气场强大”等抽象性格词替代外观，不写剧情动作，不出现第二个人，不使用在世真人或明星脸。全身人物设定图的构图和禁用项由系统在生图时统一追加，不要在每个人的 image_prompt 中重复。`,
    ...
    episode: `撰写指定单集微短剧。这些是不随模板切换的内置底线：只写观众能看到或听到的内容；不写文学性心理、作者解释、主题概括、镜头参数、导演指令、演员教学或时长说明；不为凑篇幅堆砌转头、点头、皱眉、握拳等无效微动作；硬性禁用瞳孔骤缩、眼睛一亮、眼中闪过、眼神一沉/一凛、嘴角上扬、眉头一皱、脸色一变、倒吸凉气等套路化反应词。握紧拳头不是禁词，但只有它会改变交锋或推动剧情时才写。情绪只在观众必须知道人物当前状态时，用身体发抖、表情惊慌、动作停下等朴素可见结果表达；动作、环境、道具、反应和对白必须用于推进事件、传递必要信息、改变关系或制造危机；不重复上一集已完成的事件，不提前泄露后续集的信息；本集必须发生明确的状态变化，结尾停在新危险、新发现、新选择或强烈反应上。具体的 EP 标题、场次格式、篇幅和声音标注服从当前选中的模板；实际场次数量服从已经确认的剧情安排，不得为模板目标合并或补造场次。文风、情绪、节奏和剧情逻辑始终服从内置创作规则，不受模板影响。任何圆括号或小括号始终禁止，模板不得覆盖。只输出剧本成稿。`,
...
最终剧本是本次提炼唯一允许使用的剧情事实来源；不得读取、补充或推断小说中间稿独有的信息。每个 subject 和 value 都必须是脱离当前页面仍能独立理解的状态快照，禁止出现“本集”这种相对指代；直接写已经成立的事实、当前结果或尚未解决的问题，不写“本集新增、本集验证、本集结束时”等过程标签。任何推测性信息都不得作为事实或人物档案记录，包括带有“看起来、疑似、似乎、好像、可能是”等含义的判断；只有最终剧本或已有账本明确确认后才能记录。当前账本中未变化的项目不要重复输出，但剧本中新出现的姓名、年龄、病症、学校、职位等精确属性属于对旧条目的有效更新，不能当作“没有变化”而略过。必须逐条检查已有 unresolved：若最终剧本已明确展示索赔被撤回、威胁者退让且当前威胁解除、债务付清、目标完成或问题以其他可见结果结束，必须沿用已有 category 与 subject 输出 resolved；只是暂时离场、口头拖延、风险仍可能继续时不得判定解决。要替换或补全旧状态时，先以相同 category 与 subject 输出 replaced，再以相同 category 与 subject 和包含新旧有效信息的新 value 输出 active。新增 subject 使用短而稳定的实体或事项名称，避免换同义称呼。通常0–20项，人物和系统项不得因项数上限被省略；没有值得延续的内容则返回空数组。`,
  ...
  const characterBrief=stage==="characters"?String(artifacts.find(item=>item.type==="character_brief")?.content?.text||"").trim():"";
  const characterDirective=characterBrief?`\n\n【本次必须优先落实的人物补充要求】\n${characterBrief}\n\n生成前在内部逐项拆解其中的新增人数、总人数、性别、角色定位、身份、关系及对旧人物的修改要求；先逐项落实，再补充主线所需人物。不要输出核对过程。若要求“新增/再加/少了N个”，必须在已有不同姓名的人物之外另建N张独立人物卡；不得用改名、改角色标签、群体称谓或已有角色充数。此段是本次人设生成的直接指令，优先于模型自行判断的人物数量与配置。`:"";
  return `${shared}${engine}\n\n当前任务：${tasks[stage]}\n\n项目上下文：\n${context(stage, project, constraints, artifacts, evidence, range)}${characterDirective}`;
}
...
EP编号、“第X集”“本集”“上集”“下集”等只属于梗概策划层，可以用于编辑说明，但绝不能进入故事世界。角色台词、日记、信件、纸条、短信、录音、广播、弹幕、系统提示、预言及其他故事内信息载体不得引用或感知本作品的集数，也不得借集数无依据预知未来剧情；若人物需要获知将来的危险，必须改用故事内已经成立的日期、约定、证据或威胁，没有依据则删除这项预知。
...
未到规划首登场集的主要人物，其人设只作为未来边界，不得让其在当前主线现场行动、说话、实时联系主角或无依据介入事件；别人提及其姓名，或其仅出现在照片、档案、既有录音录像、新闻、梦境及明确闪回中，不算当前主线正式出场。实时通话、直播连线或主动介入当前事件仍算正式出场。当集标为首次出场的人物必须由本集既定事件自然引入，不能为完成名单硬塞无关镜头。已经首登场的人物也不等于本集必须出现，仍以本集大概内容与必须发生为准。
...
【剧情安排】
先从小说第一段扫描到最后一段，按实际发生顺序建立场次，不预设场次数量，也不得为了压缩而遗漏真实场景。人物进入新的独立地点，并在该处发生对白、冲突、交易、发现、取放关键道具或其他有效剧情，就必须另起一场；内外景改变也必须另起一场。同一地点内连续发生的剧情合并为一场。仅路过、望向门口、站起、走几步或未在新地点发生有效剧情的移动，不单独成场。

每场严格写两行。第一行是“连续序号 外/内 具体地点 实际时段”，例如“1 外 城中村巷道 凌晨”；只能使用一个地点，禁止斜杠、并列地点和括号摘要。第二行只用一对圆括号，把该地点实际发生的全部有效事件按小说原顺序写成事件链。圆括号内按独立事件节点书写，节点之间统一用“+”连接；逗号只用于单个节点内部。事件链写清人物初始位置、当前动作、关键交锋、位置变化、道具动线和结果，不写装饰性环境；不得遗漏、重复、改序或新增小说事件。时间明显变化但地点不变时，若仍属连续行动可保留同场并在事件链中写明时间经过；若已形成新的独立行动阶段，则另起场次。最后一场的最后一个事件必须是既定cliffhanger。

输出前必须反向核对整篇小说：每一处真正承载剧情的地点是否都有独立场次，每个场次是否都能在小说中找到依据，每个有效事件是否落入正确场次。场次数量由小说实际场景决定，不能为了凑数量拆出空场，也不能为了减少数量合并不同地点。
再逐场检查圆括号事件链：如果中途出现进入店铺、回到住处、抵达公司等明确完成空间落地的动作，必须从该动作开始另起场次，不能继续留在原地点标题下；进入动作若是该场第一项，则作为本场落地动作正常保留。推开、拉开或关上一扇门本身不等于换场，人物可能仍在原位置对话，不得仅凭开关门拆场。
...
【程序定位的转场核对清单｜逐条必须判断】
${continuity.sceneTransitionCandidates?.length?continuity.sceneTransitionCandidates.map((item,index)=>`${index+1}. ${item}`).join("\n"):"清单为空，仍须自行完整扫描小说"}
以上原文位置必须逐条核对，不得跳过。若人物在该处实际进入新地点，并在新地点继续发生有效剧情，该事件必须成为新场次第一项；若只是提及地点、计划前往、回忆、电话内容，或没有在新地点发生有效剧情，才可判定为非转场。不得默默忽略任何一项，也不得把确认为转场的事件继续用“+”连接在旧地点场次中。该清单用于强制判断，不是强制场次表；不得据此补造事件，也不得忽略清单之外的真实地点转换。

小说中间稿（保持原文，未插入任何内部标记）：
${episode?.novel||""}
...
输出前核对：每个场次只有“标题一行＋圆括号事件链一行”；场次从1开始连续编号；标题中不得原样出现“具体地点”“时间”或“时段”，不得出现斜杠、并列地点或剧情摘要；括号事件链中不得出现场次标题。只输出四个【】区块，不输出边界检查、人物标识符、Markdown或额外解释。`;
}
...
export function buildSkillEpisodePrompt(project, constraints, artifacts, episode, states, templateGuide, continuity = {}) {
  ...
  const lockedSceneHeadings=String(continuity.episodePlan||episode?.episode_plan||"").split(/\r?\n/).map(line=>line.trim()).filter(line=>/^\d+\s+(?:内|外|内景|外景)\s+.+$/.test(line));
  return `${base}
...
剧本的主角身份不得因其他人物戏份较多而改变；只有上述锁定主角可以承接小说视角与必要的 V.O.。无论小说采用第一人称还是第三人称，主角 V.O. 一律改写成主角本人的第一人称口吻：自称使用“我/我的”，不得用主角姓名或“他/她”称呼自己；指向其他人物的姓名和代词保持原指向，不得一并替换。
...
【剧情安排时空路线｜事件顺序必须保留，场次数可按表演节奏调整】
${lockedSceneHeadings.join("\n")||"未识别到场次标题"}
上述路线用于锁定事件先后、地点与时间变化。剧本可因连续表演节奏合理拆场或合并相邻且时空连续的短场，但不得遗漏地点转换、改变事件顺序或丢失对应正文。
...
1. 最终剧本必须执行已经确认的【剧情安排】，严格保留事件先后、地点路线和具体时段，但场次数量不要求与剧情安排机械相等。可按实际表演节奏拆开确有时空转换的内容，也可合并相邻且时空连续、合并后仍清楚可拍的短场；不得遗漏地点转换、换序或把“下午一点多、晚、次日清晨”等时段粗化为“日/夜”。场次从1开始连续编号。默认模板标题写成“2 外 城中村街道 下午一点多”，只统一使用外/内，不写外景/内景。第三人称；对白写“人物：台词”且每句独立成段；系统播报写“系统音：”；只有主角在必要时可用“主角名 V.O.：”。
1.1 剧情安排各场的圆括号事件链都必须按原顺序展开为必要动作和充分对白。允许为可拍摄性调整分场边界，但转换不得改变、删减、重复或跨序挪动事件、对白、因果与钩子。
...
2.1 不得只靠“沉思、心想、意识到”等提示词判断心理。小说中连续两句及以上若主要在回顾已知信息、盘点人物与压力、权衡选择或形成决定，而不是描写现场可见动作，就把整段作为一个心理信息块处理。只提取会改变主角当前行动的必要认知，合并、压缩并口语化为一至两句“${protagonist} V.O.：台词”；重复前情、逐项念故事账本和泛泛的励志总结直接删除。不得把原段逐句转换成多条 V.O.，也不得因段中提到其他人物，就把其他人物的内心或未说出口的话交给主角复述。
...
6. 禁止环境、天气、氛围、时间推进、比喻、感官、情绪外化、声音语气、旁白总结、不说话/盯着看、“脸一阵青一阵白”等状态与套路化神态描述。禁用“指节发白、手指关节发白、青筋暴起、指甲掐进掌心、掌心渗出血丝、手背筋络凸起”等套路化 AI 身体细节；无事件作用时直接删除，确有关键动作时只保留朴素动作，不得换成另一种装饰性反应。禁止单独写“某人走了”“某人来了”这种没有表演过程的结果句；若进出场会影响当前交锋，应写成可拍摄的具体动作，例如“刘麻子头也不回地走了”“王志远推开人群走进来”“两个保安架着黄毛离开”。每场结尾必须出现推动下一场的新危机或冲突，只有最后一场使用cliffhanger。
...
排版硬规则：不输出 EP 集标题，直接从第1个场次开始；每个场次标题单独一行；每句“人物：台词”单独一行；动作不得与台词混在同一行。两句人物台词之间如果有多句或多段连续动作，必须全部合并成一个动作段、只占一行，不能把每个动作句各拆一行。同一行不得出现两个人物台词；不同类别之间保留换行，不得把整篇正文挤成一段。
...
模板只控制格式、篇幅与场次呈现方式，不覆盖剧情安排已经确定的实际场次数量，也绝不控制文风、情绪、节奏或剧情逻辑；样例只用于核对排版，故事和表达方式不得照搬：
...
只输出从第1个场次开始的完整剧本；不输出EP标题、分析、Markdown、字数、“EP完”或修订说明。`;
}
