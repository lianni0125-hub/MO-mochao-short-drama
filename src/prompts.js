...
export function buildEpisodeNovelPrompt(project, constraints, artifacts, episode, states, templateGuide, continuity = {}) {
  ...
  const openingRule=Number(episode?.episode_no||1)===1
    ...
    : `这是EP${String(episode?.episode_no||1).padStart(2,"0")}。本集【必须发生】节点1是唯一的开篇剧情任务，通常就是上一集钩子的现场兑现；第一段必须直接开始展开节点1，尽快写出行动、交锋与可见结果，不得先另写承上启下事件、回顾段或自行设计的过渡剧情。先读取【上一集小说连续性概要】理解上一章的完整因果、人物处境和结尾局面，再按编号从小到大读取【上一集最近事件】校正实际发生的顺序与硬事实；两者冲突时以最近事件为准。最近事件列表中已有结果的动作均视为完成，禁止重新执行、重新鉴定、重新揭露或换一种说法重演。【上一集小说末尾】最后3–5行只用于保持人物位置、尚未完成的即时动作和第一人称语气，不是需要续写或复述的剧情任务；若其与最近事件冲突，以最近事件为准。只有完成节点1必不可少、且属于节点1自身过程的最少动作可以放在结果之前，不得在节点1前增加独立事件。不复述上一集，不重复钩子，不使用上集说到、却说、话说等说书腔。`;
...
硬规则：
1. 【必须发生】已经拆成从“节点1”开始的顺序检查点。先在内部建立节点1至最后节点的覆盖清单，再按编号逐项写作，但正文不得输出节点编号或清单。开篇必须直接进入节点1，绝不能从节点2或更后面的事件切入；当前节点的人物行动、交锋与明确结果尚未写成之前，禁止进入、借用或提前暗示后续节点。每个节点至少形成一个完整的戏剧单元，写清目标、阻力、应对和可见结果；该结果再触发下一节点。不得删节点、交换顺序、把相隔节点揉成一件事，或用一句回忆、概述、事后转述代替现场展开。最后节点就是钩子，写到此处立即停止，不加环境余韵、过渡、总结或下一步结果。
2. 严格避开【不得揭示】；不新增会改变主线、能力规则、人物关系或后续走向的事实。
3. 严格执行上方【小说开头技巧】。所有集都从节点1直接开戏；EP02以后，上一集末尾与最近事件只约束节点1开始时的人物位置、认知、道具和已完成事项，不构成节点1之前的额外剧情。只有上一集尚未完成的即时动作本身就是节点1的一部分时，才在节点1内部继续该动作。
4. ${intensity}
5. 完整照做 Skill 的小说扩充原则：【必须发生】只规定本集不可缺少、不可乱序的事件骨架，不限制每个节点内部的具体戏剧设计。必须先完成当前节点，再在当前节点结果与下一节点之间设计自然转场。请在不改变事件顺序、既定事实、人物认知、人物关系、能力规则、道具状态和后续边界的前提下，天马行空、大胆主动加戏；通过具体事件和行为深挖人设，把性格真正设计成情节。表现层可以自由创造并充分展开对白交锋、现场阻力、临时策略、冲突升级、对手反扑、笑点、情感张力、可见后果和不影响连续性的软细节，使成稿明显超越事件链的逐条复述。每一处新增内容都必须归属于当前正在执行的某个节点，并能在本集内解释清楚或收束；“自由加戏”绝不代表可以另选开头、绕过节点或打乱节点。
...
5.2 严格遵守人物认知边界。人物在任一时刻只能依据此前已经亲眼看见、亲耳听见、被明确告知或亲自查证的信息作出反应；【必须发生】中稍后才会兑现的揭露、打脸、身份、结果和钩子，不是开场事实，绝不能倒推成该人物已经知道。不得在缺少前置获知事件时写“被骗后的懊恼”“得知真相后的愤怒”“早已明白”“像是知道了”等状态。先写清信息如何到达该人物，再写其即时反应；没有证据时只写当下可观察动作、现场对白或中性的疑惑与警惕。非主角的内心认知不得由第一人称叙述者直接断言。
...
10. 写完后严格执行 Skill 自检并在内部直接修正：把成稿逐段反向对应节点1至最后节点，确认每个节点都能在正文中找到现场发生的行动、交锋和结果，且正文首次出现各节点的顺序与编号完全一致；任何节点缺失、只有概述、顺序颠倒或后项抢跑，都必须在输出前修正。再检查每个事件的前因后果、能力如何触发且是否符合设定、对手反应是否符合利益、每段情绪是否自然递进、结尾是否准确停在链条最后一个钩子。输出前检查全部标点：圆括号和小括号必须为零；英文双引号及中文弯双引号必须为零；每一句对白和声音都必须使用「」并成对闭合。括号内信息必要就直接融入自然叙述，不必要就删除。钩子必须体现主角已经决定或开始行动，而不是刚发现机会、继续挨欺负或站着发呆。不要输出自检过程。
...
【资料分工】
1. 上一章小说全文用于理解完整因果、人物动机、现场处境和第一人称叙事中已经设计的过程。
