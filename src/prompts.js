...
1. 最终剧本必须为1–9个正式场次并执行剧情安排。同一地点且同一时间的连续交锋不得为了凑数拆场；地点或主时间明确变化时必须另起正式场次。场次标题严格服从当前模板；默认模板写成“2 外 城中村街道 日”，只能使用外/内和日/夜，不写外景/内景、白天/夜晚。第三人称；对白写“人物：台词”且每句独立成段；系统播报写“系统音：”；只有主角在必要时可用“主角名 V.O.：”。内心独白必须直接写成“主角名 V.O.：台词”，严禁先写“在心里说、心中默念、内心说道”等动作说明，再另起一行写成普通对白。
1.1 剧情安排中的1–3个编号是按顺序组织剧情的推进段，不是最终剧本场次编号。若推进段标题带“→”、地点跨度或时间跨度，必须按标题顺序把每个明确地点或主时间拆成独立的正式场次，再从1开始为全篇连续编号；不得继续保留“地点A→地点B”的复合场次总标题，也不得使用“【内 地点 时间】”“【外 地点 时间】”作为内部转场。拆场只改变分场结构，不得改变、删减或重复原有事件、对白、因果与钩子；同一地点和时间的内容仍须合并。
...
6. 禁止环境、天气、氛围、时间推进、比喻、感官、情绪外化、声音语气、旁白总结、沉默/不说话/盯着看、“脸一阵青一阵白”等状态与套路化神态描述。禁用“指节发白、手指关节发白、青筋暴起、指甲掐进掌心、掌心渗出血丝、手背筋络凸起”等套路化 AI 身体细节；无事件作用时直接删除，确有关键动作时只保留朴素动作，不得换成另一种装饰性反应。禁止单独写“某人走了”“某人来了”这种没有表演过程的结果句；若进出场会影响当前交锋，应写成可拍摄的具体动作，例如“刘麻子头也不回地走了”“王志远推开人群走进来”“两个保安架着黄毛离开”。每场结尾必须出现推动下一场的新危机或冲突，只有最后一场使用cliffhanger。
