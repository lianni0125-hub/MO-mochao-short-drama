...
const causalBridgeRules=`因果桥硬规则：直接造成后续系统激活或提示、能力触发、人物受伤、道具变化、危险升级或下一关键行动的动作，是必须保留的因果桥事件；即使很短，也不得因后续结果更显眼而略过。输出前反查每个系统、能力、受伤、道具或重要揭示事件的直接前因；前因出现在当前片段时必须单独提取。普通碰触、无结果动作和气氛不因此收录。`;
...
const temporalPatterns=[
  ...
  {kind:"duration",precision:"relative",regex:new RegExp(`(?:过去|此前|最近|近|前|这|未来)(?:[${cnNumber}]+|几)\\s*(?:分钟|小时|天|日|周|星期|个月|月|年)`,"g")},
  ...
  {kind:"duration",precision:"relative",regex:new RegExp(`[${cnNumber}]+\\s*(?:分钟|小时|天|日|周|星期|个月|月|年)(?:了|来|以来)?`,"g")},
...
function temporalRelationFor(marker){
  if(/(?:与此同时|同一时间|同一时刻)/.test(marker))return "same_time";
  if(/(?:以内|之内|内)(?:的)?(?:凌晨|清晨|早晨|早上|上午|中午|下午|傍晚|晚上|夜里|深夜)?$/.test(marker))return "within";
  if(/(?:大前年|前年|去年|大前天|前天|昨天|不久前|早些年|那一年|那年|当年|片刻前|上上个|上个|过去|此前|最近|近|前(?:几|[一二两三四五六七八九十百千〇零0-9])|以前|之前|前(?:的)?(?:凌晨|清晨|早晨|早上|上午|中午|下午|傍晚|晚上|夜里|深夜)?$|十年前|多年前|小时候|童年)/.test(marker))return "before";
  if(/(?:未来|明年|后年|明天|后天|大后天|次日|翌日|后来|没过多久|过了一阵|一段时间后|片刻后|不一会儿|转眼间|多年以后|多年之后|许多年后|下个|下下个|以后|之后|后(?:的)?(?:凌晨|清晨|早晨|早上|上午|中午|下午|傍晚|晚上|夜里|深夜)?$)/.test(marker))return "after";
  if(/^(?:这)?(?:[一二两三四五六七八九十百千〇零0-9]+|几)\s*(?:分钟|小时|天|日|周|星期|个月|月|年)(?:了|来|以来)?$/.test(marker))return "duration";
  return "at";
}
const temporalMarkersFor=row=>{
  const stored=cleanStoredTime(row?.time_text),evidence=[row?.source_quote].map(text).filter(Boolean),markers=[];
...
function temporalRelationRole(event,marker){
  const evidence=text(event?.source_quote),value=text(marker?.marker_text);
  if(/(?:以内|之内|截止|期限|限期|到期)/.test(`${value} ${evidence}`)&&/(?:必须|需要|要在|交|缴|还|付|完成|赶到|截止|期限|限期|到期)/.test(evidence))return "deadline";
  if(event?.timeline_type==="flashback")return "historical_reference";
  if(event?.timeline_type==="flashforward")return "future_reference";
  if(event?.timeline_type==="parallel"||marker?.relation==="same_time")return "parallel";
  return "occurrence";
}
...
dimensionSchema.properties.resources={type:"array",items:{type:"object",properties:{order:{type:"integer"},owner:{type:"string"},name:{type:"string"},kind:{type:"string",enum:["流动资金","世界内货币","不动产","动产","股权产业","债权","债务","待兑现收益","可消耗资源","其他资源"]},amount_text:{type:"string"},unit:{type:"string"},availability:{type:"string",enum:["可支配","受限","冻结","待到账","不可直接变现","已耗尽","未知"]},current_state:{type:"string"},change_type:{type:"string",enum:["首次出现","获得","支出","购买","出售","转让","抵押","冻结","解冻","借入","偿还","遗失","被夺","消耗","增值","贬值","承诺未到账","状态变化","无状态变化"]},change_summary:{type:"string"},source_quote:{type:"string"}},required:["order","owner","name","kind","amount_text","unit","availability","current_state","change_type","change_summary","source_quote"],additionalProperties:false}};
dimensionSchema.required.push("resources");
...
export function memoryStats(projectId){
  ...
  const relationships=get("SELECT COUNT(*) count FROM memory_relationships WHERE project_id=@id",{id:projectId})?.count||0,secondaryCharacters=get("SELECT COUNT(*) count FROM memory_secondary_characters WHERE project_id=@id AND active=1",{id:projectId})?.count||0,goldenFingers=get("SELECT COUNT(*) count FROM memory_golden_fingers WHERE project_id=@id AND active=1",{id:projectId})?.count||0,goldenAbilities=get("SELECT COUNT(*) count FROM memory_golden_abilities WHERE project_id=@id AND active=1",{id:projectId})?.count||0,importantProps=get("SELECT COUNT(*) count FROM memory_important_props WHERE project_id=@id AND active=1",{id:projectId})?.count||0,resources=get("SELECT COUNT(*) count FROM memory_resources WHERE project_id=@id AND active=1",{id:projectId})?.count||0;
  ...
  return {status:entities||events?"ready":"empty",entities:Number(entities),events:Number(events),links:Number(links),chains:Number(chains),embedded:Number(embedded),relationships:Number(relationships),secondaryCharacters:Number(secondaryCharacters),goldenFingers:Number(goldenFingers),goldenAbilities:Number(goldenAbilities),importantProps:Number(importantProps),resources:Number(resources),temporalEvents:Number(temporalEvents),flashbackEvents:Number(flashbackEvents),embeddingConfigured:embeddingConfigured(),syncedEpisode:Number(synced),updatedAt:updated,lastExtraction:extraction};
}

export function memorySnapshot(projectId){
  if(get("SELECT COUNT(*) count FROM memory_temporal_relations WHERE project_id=@id AND anchor_event_id=event_id",{id:projectId})?.count)rebuildTemporalRelations(projectId);
...
export function rebuildTemporalRelations(projectId,episodeNo=null){
  ...
  return transaction(()=>{
    ...
    const events=all(`SELECT * FROM memory_events WHERE ${where} AND active=1 ORDER BY episode_no,event_order`,params);let previousMain=episodeNo==null?null:all("SELECT * FROM memory_events WHERE project_id=@pid AND active=1 AND episode_no<@episode AND timeline_type IN ('main','parallel') ORDER BY episode_no DESC,event_order DESC LIMIT 1",{pid:projectId,episode:episodeNo})[0]||null,count=0;
    for(const event of events){const markers=temporalMarkersFor(event);for(const marker of markers){const role=temporalRelationRole(event,marker),anchor=role==="deadline"?null:previousMain,relation=role==="deadline"?"within":event.timeline_type==="flashback"?"before":event.timeline_type==="flashforward"?"after":event.timeline_type==="parallel"?"same_time":marker.relation,anchorLabel=anchor?`EP${String(anchor.episode_no).padStart(2,"0")}事件${anchor.event_order}：${anchor.summary}`:role==="deadline"?`EP${String(event.episode_no).padStart(2,"0")}事件${event.event_order}设立的期限`:marker.precision==="exact"?`明确时间点：${marker.marker_text}`:"尚无更早可连接事件",targetLabel=role==="deadline"?`${marker.marker_text}期限点`:"",evidence=text(event.source_quote);if(anchor?.id===event.id)continue;run(`INSERT OR IGNORE INTO memory_temporal_relations(project_id,event_id,episode_no,marker_order,marker_text,relation,anchor_event_id,anchor_label,timeline_type,marker_kind,precision,relation_role,target_label,source_quote,updated_at) VALUES(@pid,@event,@episode,@order,@marker,@relation,@anchor,@label,@timeline,@kind,@precision,@role,@target,@quote,@time)`,{pid:projectId,event:event.id,episode:event.episode_no,order:marker.marker_order,marker:marker.marker_text,relation,anchor:anchor?.id||null,label:anchorLabel,timeline:event.timeline_type||"main",kind:marker.kind,precision:marker.precision,role,target:targetLabel,quote:evidence,time:now()});count++;}if(["main","parallel"].includes(event.timeline_type))previousMain=event;}
...
function eventVectorText(item){return [text(item.summary),text(item.time_text)&&`时间：${text(item.time_text)}`,text(item.location)&&`地点：${text(item.location)}`,text(item.qualifier_text)&&`限定：${text(item.qualifier_text)}`,text(item.result_text)&&`结果：${text(item.result_text)}`,text(item.temporal_anchor)&&`时间锚点：${text(item.temporal_anchor)}`].filter(Boolean).join("｜");}
...
async function ensureEventEmbeddings(projectId,episodeNo,signal){
  ...
  const events=all("SELECT id,episode_no,event_order,summary,time_text,location,qualifier_text,result_text,temporal_anchor,embedding_json,embedding_model FROM memory_events WHERE project_id=@pid AND active=1 AND episode_no<@episode ORDER BY episode_no,event_order",{pid:projectId,episode:episodeNo}),stored=new Map(all("SELECT source_id,text_content FROM memory_vectors WHERE project_id=@pid AND memory_type='plot_event' AND embedding_model=@model",{pid:projectId,model:provider.model}).map(item=>[Number(item.source_id),text(item.text_content)]));
  const missing=events.filter(item=>!parseVector(item.embedding_json).length||item.embedding_model!==provider.model||stored.get(Number(item.id))!==eventVectorText(item));let updated=0;
  for(let offset=0;offset<missing.length;offset+=64){const batch=missing.slice(offset,offset+64),contents=batch.map(eventVectorText),vectors=await embedTexts(contents,{signal});for(let index=0;index<batch.length;index++){run("UPDATE memory_events SET embedding_json=@vector,embedding_model=@model,embedded_at=@time WHERE id=@id",{vector:JSON.stringify(vectors[index]),model:provider.model,time:now(),id:batch[index].id});upsertVector(projectId,"plot_event",batch[index].id,batch[index].episode_no,contents[index],vectors[index],provider.model);updated++;}}
  return updated;
}
...
function uncoveredTailAndSystemLines(script,events){
  ...
  const temporal=lines.map(item=>({...item,temporalMarkers:explicitTimesFrom(item.line).filter(marker=>!events.some(event=>{const position=sourcePosition(script,event.source_quote);return position.start<=item.lineNo&&position.end>=item.lineNo&&temporalMarkersFor(event).some(saved=>saved.marker_text===marker);} ))})).filter(item=>item.temporalMarkers.length);
  return [...new Map([...system.map(item=>({...item,reason:"system"})),...tail.map(item=>({...item,reason:"tail"})),...temporal.map(item=>({...item,reason:"temporal"}))].map(item=>[item.lineNo,item])).values()];
}
...
${unit.text}`;
  ...
  for(let repairRound=1;repairRound<=2&&stillUncovered.length;repairRound++){
    ...
    const wanted=new Set(stillUncovered.flatMap(item=>{const indexes=[];for(let i=Math.max(1,item.lineNo-2);i<=Math.min(lineRows.length,item.lineNo+2);i++)indexes.push(i);return indexes;})),repairText=[...wanted].sort((a,b)=>a-b).map(no=>`${no}：${lineRows[no-1]}`).join("\n"),repairTargets=stillUncovered.map(item=>`第${item.lineNo}行｜${item.reason==="temporal"?`时间补漏｜必须保留：${item.temporalMarkers.join("、")}`:item.reason==="system"?"系统信号补漏":"集尾补漏"}｜${item.line}`).join("\n");
    const previousFailure=repairRound===2?`\n\n【上一轮仍未覆盖】\n${stillUncovered.map(item=>`第${item.lineNo}行${item.temporalMarkers?.length?`（必须保留时间：${item.temporalMarkers.join("、")}）`:""}：${item.line}`).join("\n")}\n上一轮没有正确覆盖这些具体内容。本轮必须为每个仍缺失的系统变化、集尾钩子或明确时间事实返回事件；不得只改写已有事件。`:"";
    const coveragePrompt=`你是剧情记忆库的定点补漏员。【必须补漏的精确目标】才是任务清单，上下文只用来理解目标，不得用上下文中的其他句子代替目标。系统信号和集尾钩子必须返回。对每个“时间补漏”目标，只要该句表达的经过、期限、约定、持续状态或历史信息能影响后续理解，就为它返回事件；将目标中标明的原文时间逐字写入time_text，且source_quote必须直接包含该时间。对明显只是辱骂、夸张或无法确认真实性的时长，可以不建事件。不得重复已有事件；source_quote必须逐字取自剧本正文，不包含前置行号。\n\n${memoryEventRules}\n\n【必须补漏的精确目标】\n${repairTargets}\n\n【已有事件】\n${JSON.stringify(merged.map(item=>({summary:item.summary,source_quote:item.source_quote})),null,2)}\n\n【仅供理解的上下文】\n${repairText}${previousFailure}`;
  ...
  const hardUncovered=stillUncovered.filter(item=>item.reason!=="temporal");
  if(hardUncovered.length&&audit.provider!=="mock")throw new Error(`剧情记忆覆盖验收未通过：剧本第 ${hardUncovered.map(item=>item.lineNo).join("、")} 行的集尾或系统信号仍未提炼；已保留原记忆，请重试`);
  ...
  let lockedCharacterIdentifiers=[];
  try{lockedCharacterIdentifiers=JSON.parse(episode.character_identifiers_json||"[]");}catch{}
  const genericCharacterIdentifier=/^(?:中年男人|中年女人|男人|女人|司机|医生|护士|街痞|混混|灰色背影|神秘人|路人|保安|老板|珠宝店老板|系统音)$/;
  const stableCharacterIdentifiers=[...new Set(lockedCharacterIdentifiers.map(text).filter(name=>name&&!genericCharacterIdentifier.test(name)&&!initialNames.has(name)&&new RegExp(`(?:^|\\n)${name.replace(/[.*+?^${}()|[\\]\\]/g,"\\$&")}[：:]`,"m").test(episode.script)))];
  const priorRelationships=all("SELECT person_a,person_b,current_state,latest_episode FROM memory_relationships WHERE project_id=@pid",{pid:project.id}),priorGolden=all("SELECT canonical_name,kind,owner,fixed_rules,current_state,latest_episode FROM memory_golden_fingers WHERE project_id=@pid AND active=1",{pid:project.id}),priorAbilities=all("SELECT golden_name,canonical_name,owner,description,status,conditions,replaces_ability,latest_episode FROM memory_golden_abilities WHERE project_id=@pid AND active=1",{pid:project.id}),priorProps=all("SELECT canonical_name,kind,significance,current_holder,current_location,origin_text,current_state,latest_episode FROM memory_important_props WHERE project_id=@pid AND active=1",{pid:project.id}),priorResources=all("SELECT owner,canonical_name,kind,amount_text,unit,availability,current_state,latest_episode FROM memory_resources WHERE project_id=@pid AND active=1",{pid:project.id});
  const dimensionPrompt=`你是长篇短剧记忆库的分类提炼员。唯一事实来源是下面本集最终剧本。只提炼人物关系变化、剧情中新出现的实名次要人物、主角金手指及变化、重要道具及变化、资源账本及变化。
...
- “中年男人、司机、灰色背影、街痞、医生、珠宝店老板”等泛称、职业或临时称呼不建档；“王叔”这类已锁定并实际作为说话人标识的稳定称呼可以建档。
- 下方“本集锁定人物标识”中出现并在剧本里实际说话的名字，除03初始人物外必须逐一建档，不得漏掉。
- 精确人物标识默认代表不同人物。即使同姓、年龄或身份描述相近，也不得把“王叔”与“王志远”等两个标识合并；只有剧本明确说明二者是同一人的别名时才能归一。
...
【资源账本】
- 只记录主角明确拥有、控制、负担或待收取，且会影响行动能力的可量化资源与资产：现金、账户余额、世界内货币、房产、车辆、股份产业、债权、债务、待到账报酬和重要可消耗物资。普通随身物品与剧情证物归重要道具，不在这里重复；系统积分继续归金手指，除非它明确可交易、转让或作为通用货币使用。
- owner与name共同确定同一资源。name使用稳定名称，如“人民币个人可支配资金”“云湖别墅产权”“玄鉴币”；个人资产、公司资产、现金、房产估值和待到账承诺不得合并。
- amount_text只写剧本明确确认的当前数量、份额或“金额未明确”，不得猜测日常开销、价格、估值或余额。unit写元、枚、套、%、吨等明确单位；availability区分可支配、受限、冻结、待到账、不可直接变现、已耗尽或未知。
- current_state写本集结束时的所有权、控制权、负债、用途限制和可用性。承诺给钱不等于到账，房产价值不等于现金，借款增加现金时也要保留债务。只有原文同时给出旧额、增减额且单位一致时才能计算结果，否则保留“当前数额未明确”，禁止补算。
- 首次出现或发生获得、支出、购买、出售、转让、抵押、冻结、解冻、借入、偿还、遗失、被夺、消耗、增贬值、承诺未到账等变化时输出。change_summary写清“原状态→明确事件→本集结束状态”；没有变化只被提及时change_type=无状态变化且change_summary留空。
- 旧资源默认延续，不得因本集未提及就删除或归零。倒叙、未来预示和假设内容不能覆盖当前账本。source_quote必须直接证明本集变化或当前数值。
...
【本集锁定人物标识（稳定说话人候选）】
${JSON.stringify(stableCharacterIdentifiers)}
...
【提炼前资源账本】
${JSON.stringify(priorResources,null,2)}
...
${episode.script}`;
  ...
  if(raw.length&&embeddingConfigured()){
    ...
    await ensureEventEmbeddings(project.id,episode.episode_no,signal);eventVectors=await embedTexts(raw.map(eventVectorText),{signal});
...
10. 已提炼的碰撞、袭击、受伤、触碰、打开或使用等因果桥动作，若直接引出后续系统激活、能力触发、道具变化或危机结果，必须用“导致”、“利用”或“升级”连入同一剧情链；不得只连后续系统播报而丢掉已有的直接前因。
...
${JSON.stringify(candidates.map(item=>({id:item.id,episode_no:item.episode_no,event_order:item.event_order,summary:item.summary})),null,2)}`;
  ...
  transaction(()=>{
    ...
    const oldRelationshipIds=all("SELECT id FROM memory_relationship_changes WHERE project_id=@pid AND episode_no=@episode",{pid:project.id,episode:episode.episode_no}).map(item=>item.id),oldGoldenIds=all("SELECT id FROM memory_golden_changes WHERE project_id=@pid AND episode_no=@episode",{pid:project.id,episode:episode.episode_no}).map(item=>item.id),oldAbilityIds=all("SELECT id FROM memory_golden_ability_changes WHERE project_id=@pid AND episode_no=@episode",{pid:project.id,episode:episode.episode_no}).map(item=>item.id),oldPropIds=all("SELECT id FROM memory_prop_changes WHERE project_id=@pid AND episode_no=@episode",{pid:project.id,episode:episode.episode_no}).map(item=>item.id),oldResourceIds=all("SELECT id FROM memory_resource_changes WHERE project_id=@pid AND episode_no=@episode",{pid:project.id,episode:episode.episode_no}).map(item=>item.id);
    for(const sourceId of oldRelationshipIds)run("DELETE FROM memory_vectors WHERE project_id=@pid AND memory_type='relationship_change' AND source_id=@source",{pid:project.id,source:sourceId});for(const sourceId of oldGoldenIds)run("DELETE FROM memory_vectors WHERE project_id=@pid AND memory_type='golden_change' AND source_id=@source",{pid:project.id,source:sourceId});for(const sourceId of oldAbilityIds)run("DELETE FROM memory_vectors WHERE project_id=@pid AND memory_type='golden_ability_change' AND source_id=@source",{pid:project.id,source:sourceId});for(const sourceId of oldPropIds)run("DELETE FROM memory_vectors WHERE project_id=@pid AND memory_type='important_prop_change' AND source_id=@source",{pid:project.id,source:sourceId});for(const sourceId of oldResourceIds)run("DELETE FROM memory_vectors WHERE project_id=@pid AND memory_type='resource_change' AND source_id=@source",{pid:project.id,source:sourceId});
    run("DELETE FROM memory_relationship_changes WHERE project_id=@pid AND episode_no=@episode",{pid:project.id,episode:episode.episode_no});run("DELETE FROM memory_golden_changes WHERE project_id=@pid AND episode_no=@episode",{pid:project.id,episode:episode.episode_no});run("DELETE FROM memory_golden_ability_changes WHERE project_id=@pid AND episode_no=@episode",{pid:project.id,episode:episode.episode_no});run("DELETE FROM memory_prop_changes WHERE project_id=@pid AND episode_no=@episode",{pid:project.id,episode:episode.episode_no});run("DELETE FROM memory_resource_changes WHERE project_id=@pid AND episode_no=@episode",{pid:project.id,episode:episode.episode_no});
    ...
    const inserted=[];let temporalPrevious=all("SELECT id,episode_no,event_order,summary,timeline_type FROM memory_events WHERE project_id=@pid AND active=1 AND episode_no<@episode AND timeline_type IN ('main','parallel') ORDER BY episode_no DESC,event_order DESC LIMIT 1",{pid:project.id,episode:episode.episode_no})[0]||null;
    for(const [index,item] of raw.entries()){
      ...
      const eventId=Number(saved.lastInsertRowid),markers=temporalMarkersFor(item);
      for(const marker of markers){const role=temporalRelationRole(item,marker),anchor=role==="deadline"?null:temporalPrevious,relation=role==="deadline"?"within":item.timeline_type==="flashback"?"before":item.timeline_type==="flashforward"?"after":item.timeline_type==="parallel"?"same_time":marker.relation,anchorLabel=anchor?`EP${String(anchor.episode_no).padStart(2,"0")}事件${anchor.event_order}：${anchor.summary}`:role==="deadline"?`EP${String(episode.episode_no).padStart(2,"0")}事件${item.order}设立的期限`:marker.precision==="exact"?`明确时间点：${marker.marker_text}`:"尚无更早可连接事件",targetLabel=role==="deadline"?`${marker.marker_text}期限点`:"",evidence=text(item.source_quote);if(anchor?.id===eventId)continue;run(`INSERT OR IGNORE INTO memory_temporal_relations(project_id,event_id,episode_no,marker_order,marker_text,relation,anchor_event_id,anchor_label,timeline_type,marker_kind,precision,relation_role,target_label,source_quote,updated_at) VALUES(@pid,@event,@episode,@order,@marker,@relation,@anchor,@label,@timeline,@kind,@precision,@role,@target,@quote,@time)`,{pid:project.id,event:eventId,episode:episode.episode_no,order:marker.marker_order,marker:marker.marker_text,relation,anchor:anchor?.id||null,label:anchorLabel,timeline:item.timeline_type||"main",kind:marker.kind,precision:marker.precision,role,target:targetLabel,quote:evidence,time:now()});}
      inserted.push({id:eventId,item,sourceQuote:compact(item.source_quote)});
      if(["main","parallel"].includes(item.timeline_type))temporalPrevious={id:eventId,episode_no:episode.episode_no,event_order:Number(item.order)||index+1,summary:text(item.summary),timeline_type:item.timeline_type};
      upsertVector(project.id,"plot_event",eventId,episode.episode_no,eventVectorText(item),vector,embedding.model);
    }
    ...
    for(const item of dimension.resources){const owner=text(item.owner),name=text(item.name),scope=temporalScope(item),existing=get("SELECT id,first_episode,latest_episode FROM memory_resources WHERE project_id=@pid AND owner=@owner AND canonical_name=@name",{pid:project.id,owner,name});if(scope==="advance_current"){if(existing){if(episode.episode_no>=existing.latest_episode)run("UPDATE memory_resources SET kind=@kind,amount_text=@amount,unit=@unit,availability=@availability,current_state=@state,latest_episode=@episode,active=1,updated_at=@time WHERE id=@id",{kind:item.kind,amount:text(item.amount_text),unit:text(item.unit),availability:item.availability,state:text(item.current_state),episode:episode.episode_no,time:now(),id:existing.id});}else run("INSERT INTO memory_resources(project_id,owner,canonical_name,kind,amount_text,unit,availability,current_state,first_episode,latest_episode,active,updated_at) VALUES(@pid,@owner,@name,@kind,@amount,@unit,@availability,@state,@episode,@episode,1,@time)",{pid:project.id,owner,name,kind:item.kind,amount:text(item.amount_text),unit:text(item.unit),availability:item.availability,state:text(item.current_state),episode:episode.episode_no,time:now()});}if(item.change_type!=="无状态变化"&&text(item.change_summary)){const saved=run("INSERT INTO memory_resource_changes(project_id,episode_no,change_order,owner,resource_name,kind,change_type,amount_text,unit,availability,current_state,summary,source_quote,temporal_scope,updated_at) VALUES(@pid,@episode,@order,@owner,@name,@kind,@type,@amount,@unit,@availability,@state,@summary,@quote,@scope,@time)",{pid:project.id,episode:episode.episode_no,order:Number(item.order)||1,owner,name,kind:item.kind,type:item.change_type,amount:text(item.amount_text),unit:text(item.unit),availability:item.availability,state:text(item.current_state),summary:text(item.change_summary),quote:text(item.source_quote),scope,time:now()}),sourceId=Number(saved.lastInsertRowid);upsertVector(project.id,"resource_change",sourceId,episode.episode_no,item.change_summary,dimensionVectors[dimensionVectorIndex++],embeddingModel);}}
    run("DELETE FROM memory_resources WHERE project_id=@pid",{pid:project.id});
    for(const change of all("SELECT * FROM memory_resource_changes WHERE project_id=@pid AND temporal_scope IN ('advance_current','current') ORDER BY episode_no,change_order,id",{pid:project.id}))run(`INSERT INTO memory_resources(project_id,owner,canonical_name,kind,amount_text,unit,availability,current_state,first_episode,latest_episode,active,updated_at) VALUES(@pid,@owner,@name,@kind,@amount,@unit,@availability,@state,@episode,@episode,1,@time) ON CONFLICT(project_id,owner,canonical_name) DO UPDATE SET kind=excluded.kind,amount_text=excluded.amount_text,unit=excluded.unit,availability=excluded.availability,current_state=excluded.current_state,latest_episode=excluded.latest_episode,active=1,updated_at=excluded.updated_at`,{pid:project.id,owner:change.owner,name:change.resource_name,kind:change.kind,amount:change.amount_text,unit:change.unit,availability:change.availability,state:change.current_state,episode:change.episode_no,time:now()});
...
export async function compileMemoryContext(projectId,episode,signal){
  ...
  const continuityNames=[...new Set(relevantEntities.map(item=>item.canonical_name).filter(Boolean))].slice(0,8),nameText=continuityNames.join("、")||"本集主要人物";
  const retrievalQueries=[
    query,
    `${nameText}的人物连续性：当前处境、家庭成员、重要亲属、长期压力、身份、位置、健康情况，以及仍然有效且会影响行动的现实困境。`,
    `${nameText}与重要亲属、盟友、敌人和利益相关者的既往关系、长期牵挂、承诺、威胁，以及会影响当前选择的历史事件。`,
    `${nameText}涉及的系统、弹幕、技能、血统、异能、重要道具：固定规则、已验证能力、使用条件、当前数值、归属、位置和最近变化。`,
    `${nameText}当前拥有、控制、负担或待收取的资源账本：现金、账户余额、货币、房产、车辆、股份产业、债权、债务、待到账报酬、可消耗物资，以及获得、支出、出售、冻结、偿还和可用性变化。`
  ];
  ...
  let recalled=[],relationshipChanges=[],secondaryCharacters=[],goldenChanges=[],propChanges=[],resourceChanges=[];
  if(embeddingConfigured()){
    await ensureEventEmbeddings(projectId,episode.episode_no,signal);const queryVectors=await embedTexts(retrievalQueries,{signal,purpose:"query"});
    if(queryVectors.length){
      const provider=activeEmbeddingProvider(),vectorRows=all("SELECT memory_type,source_id,episode_no,text_content,embedding_json FROM memory_vectors WHERE project_id=@pid AND episode_no<@episode AND embedding_model=@model",{pid:projectId,episode:episode.episode_no,model:provider.model}).map(item=>({...item,vector:parseVector(item.embedding_json)})).filter(item=>item.vector.length);
      const rankedByQuery=queryVectors.map(vector=>vectorRows.map(item=>({...item,similarity:cosineSimilarity(vector,item.vector)})).filter(item=>item.similarity>=0.3).sort((a,b)=>b.similarity-a.similarity));
      const fuse=(type,quotas,max)=>{
        const hits=new Map();
        rankedByQuery.forEach((ranked,index)=>{for(const item of ranked.filter(row=>row.memory_type===type).slice(0,quotas[index]||0)){const saved=hits.get(item.source_id);if(!saved||item.similarity>saved.similarity)hits.set(item.source_id,item);}});
        return new Set([...hits.values()].sort((a,b)=>b.similarity-a.similarity).slice(0,max).map(item=>item.source_id));
      };
      const plotIds=fuse("plot_event",[14,6,5,5,5],20),relationshipIds=fuse("relationship_change",[3,5,6,2,2],8),secondaryIds=fuse("secondary_character",[2,6,4,1,2],8),goldenIds=fuse("golden_change",[3,1,1,8,2],8),propIds=fuse("important_prop_change",[5,2,2,3,3],8),resourceIds=fuse("resource_change",[3,3,3,2,10],10);
      const historicalRows=all(`SELECT ${fields} FROM memory_events WHERE project_id=@pid AND active=1 AND episode_no<@episode`,{pid:projectId,episode:episode.episode_no}),direct=historicalRows.filter(item=>plotIds.has(item.id)&&item.episode_no!==episode.episode_no-1),directIds=new Set(direct.map(item=>item.id));
      const adjacentIds=[];
      for(const link of all("SELECT from_event_id,to_event_id FROM memory_links WHERE project_id=@pid ORDER BY id DESC",{pid:projectId})){
        const candidate=directIds.has(link.from_event_id)?link.to_event_id:directIds.has(link.to_event_id)?link.from_event_id:null;
        if(candidate&&!directIds.has(candidate)&&!adjacentIds.includes(candidate))adjacentIds.push(candidate);
        if(adjacentIds.length>=4)break;
      }
      const adjacentSet=new Set(adjacentIds),adjacent=historicalRows.filter(item=>adjacentSet.has(item.id)&&item.episode_no!==episode.episode_no-1);
      recalled=[...direct,...adjacent].slice(0,24);
      relationshipChanges=all("SELECT * FROM memory_relationship_changes WHERE project_id=@pid AND episode_no<@episode ORDER BY episode_no,change_order",{pid:projectId,episode:episode.episode_no}).filter(item=>relationshipIds.has(item.id));
      secondaryCharacters=all("SELECT * FROM memory_secondary_characters WHERE project_id=@pid AND active=1",{pid:projectId}).filter(item=>secondaryIds.has(item.id)||query.includes(item.canonical_name));
      goldenChanges=all("SELECT * FROM memory_golden_changes WHERE project_id=@pid AND episode_no<@episode ORDER BY episode_no,change_order",{pid:projectId,episode:episode.episode_no}).filter(item=>goldenIds.has(item.id));
      propChanges=all("SELECT * FROM memory_prop_changes WHERE project_id=@pid AND episode_no<@episode ORDER BY episode_no,change_order",{pid:projectId,episode:episode.episode_no}).filter(item=>propIds.has(item.id));
      resourceChanges=all("SELECT * FROM memory_resource_changes WHERE project_id=@pid AND episode_no<@episode ORDER BY episode_no,change_order",{pid:projectId,episode:episode.episode_no}).filter(item=>resourceIds.has(item.id));
    }
  }
  ...
  const resourceKeys=new Set(resourceChanges.map(item=>`${item.owner}||${item.resource_name}`)),resources=all("SELECT * FROM memory_resources WHERE project_id=@pid AND active=1 ORDER BY latest_episode DESC",{pid:projectId}).filter(item=>resourceKeys.has(`${item.owner}||${item.canonical_name}`)||query.includes(item.canonical_name)||query.includes(item.owner));
  ...
  const resourceText=resources.map(item=>`${item.owner}｜${item.canonical_name}｜类型：${item.kind}｜当前数量/份额：${item.amount_text||"未明确"}${item.unit?` ${item.unit}`:""}｜可用性：${item.availability||"未知"}｜当前状态：${item.current_state||"无补充"}`).join("\n")+(resourceChanges.length?`\n【相关历史变动】\n${resourceChanges.map(item=>`EP${String(item.episode_no).padStart(2,"0")}：${scopeLabel(item)}${item.summary}`).join("\n")}`:"");
  ...
  const temporalRelations=all(`SELECT tr.*,e.summary event_summary,e.subject event_subject,e.action event_action,e.object_text event_object,e.result_text event_result,e.source_quote event_quote,a.summary anchor_summary FROM memory_temporal_relations tr JOIN memory_events e ON e.id=tr.event_id LEFT JOIN memory_events a ON a.id=tr.anchor_event_id WHERE tr.project_id=@pid AND tr.episode_no<@episode ORDER BY tr.episode_no,tr.event_id,tr.marker_order`,{pid:projectId,episode:episode.episode_no}),relationLabel={after:"之后",before:"之前",same_time:"同时",within:"期限内",duration:"持续",at:"时间点",unknown:"关系待定"},precisionLabel={exact:"精确",relative:"相对",vague:"模糊"};
  const settledCommitments=new Set(all(`SELECT DISTINCT l.from_event_id FROM memory_links l JOIN memory_events target ON target.id=l.to_event_id WHERE l.project_id=@pid AND target.episode_no<@episode AND l.relation IN ('兑现','替代','阻断')`,{pid:projectId,episode:episode.episode_no}).map(item=>Number(item.from_event_id)));
  const commitmentPattern=/(?:邀约|约定|赴约|期限|限期|截止|倒计时|必须|承诺|交付|交货|付款|还款|缴费|见面|集合|开庭|比赛|婚礼|任务|要求|通知|警告|威胁|决定|安排)/;
